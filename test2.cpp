...
public:
    ...
    //initialization
    Teacher(string name, string surname, unsigned int age, Subjects subject) {
        Name = name;
        Surname = surname;
        Age = age;
        Subject = subject;
    ...
    void SetMarkStudent(Student student, Subjects subject, unsigned int mark) {
        student.Give_Mark(mark, subject);
    }
};
...
int main()
{
    ...
    Teacher teacher{ "Vasyliy","Antipov",32,HISTORY };
    teacher.SetMarkStudent(student2, HISTORY, 5);
    student2.Show_Marks(HISTORY);
    return 0;
}