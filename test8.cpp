...
class Student {
    ...
    public:
        ...
        string GetName() {
           ...
           bool operator ==(const Teacher& teach2) {
       if (this->Name == teach2.Name and this->Surname == teach2.Surname and this->Subject == teach2.Subject)
           return true;
       else return false;
   }
...
public:
    ...
    bool operator ==(const Teacher& teach2) {
        if (this->Name == teach2.Name and this->Surname == teach2.Surname and this->Age == teach2.Age and this->Subject == teach2.Subject 
            and this->Mood == teach2.Mood and this->MarksCounter == teach2.MarksCounter and this->moodFactor == teach2.moodFactor)
            return true;
        else return false;
    }
...
public:
    ...
    Subjects GetLessonSubj() {
        return this->Discipline;
    }

    Teacher GetLessonTeacher() {
...
class Parent {
    ...
    public:
        ...
        //
        bool isChildMe(Student child) {
            if (this->Childrens.size() == 0)
                return false;

            for (int i = 0; i < this->Childrens.size(); i++) {
                if (child == this->Childrens[i])
                    return true;
            }
            return false;
        }
        ...
        //told about...
        virtual void ToldAboutEachOther() {
            if (Childrens.size() == 0) {
        ...
        virtual void ToldAboutSomebody() {
            int childIndex = 0;
            srand(time(0));
        ...
        virtual void ToldAboutYou(Student student) {
...
class Sobranie {
    ...
    public:
            ...
            for (int i = 0; i < Lessons.size(); i++) {
                for (int j = 0; j < Lessons[i].Students.size(); j++) {
                    ...
                    if (HaveParent == false and OldMama == false and find(ShadowChilds.begin(),ShadowChilds.end(),Lessons[i].Students[j]) == ShadowChilds.end())
                        ...
                        switch (subj) {
                        case MATH:
                            subjMarks = this->Lessons[i].Students[j].marks->Math.size();
                            break;
                        case LITERATURE:
                            subjMarks = this->Lessons[i].Students[j].marks->Literature.size();
                            break;
                        case PHYSICS:
                            subjMarks = this->Lessons[i].Students[j].marks->Physics.size();
                            break;
                        case RUSSIAN_LANGUAGE:
                            subjMarks = this->Lessons[i].Students[j].marks->Russian_language.size();
                            break;
                        case HISTORY:
                            subjMarks = this->Lessons[i].Students[j].marks->History.size();
                            break;
                        }
                        ...
                        if (subjMarks >= 1) {
                            ...
                            if (find(this->Teachers.begin(), this->Teachers.end(), Prepod) == this->Teachers.end()) {
                                ...
                                if (this->Lessons[i].Students[j].ifStudentGrateOnSubject(subj) == true)
                                    cout << "You are great teacher! My child love you so much!\n";
                                else cout << "Why you teach my child so bad ? Or it's my child problem?\n";
                            }
                        }
                    }
                }
        ...
        //вывел список детей,чьих родителей не было
            cout << "Spisok otsutstvuet parent: \n";
            for (int i = 0; i < ShadowChilds.size(); i++) {
                cout << ShadowChilds[i].GetName() << endl;
            }
        }
